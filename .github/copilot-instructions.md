...
- In `onNotificationRemoved`: remove mapping synchronously (on the calling thread) before `scope.launch` (prevents a concurrent `onNotificationPosted` re-showing the same notification from having its new mapping wiped when the remove runs later on a background thread). If the async DELETE fails (non-409), restore the old mapping only when the key is still unmapped so fallback polling can retry cleanup without clobbering any newer mapping.
...
**`postSbn` early-exit filters (SMS deduplication):** Two guards at the top of `postSbn` prevent spurious server entries when the default SMS app fires multiple `onNotificationPosted` events for a single incoming SMS: (1) **`FLAG_GROUP_SUMMARY`** — skip any notification with this flag set; it's a technical wrapper for Android notification bundling and carries no unique content. (2) **Redacted old-conversation re-post** — when a new SMS arrives, Google Messages re-posts all active conversation notifications; if an older conversation's last message was also an OTP, its notification has a blank title (Android 15 redaction). For the default SMS app, if `fetchSms()` returns null AND `extrasTitle.isBlank()`, skip the post rather than forwarding the redacted placeholder. Together these guards leave exactly one server entry per incoming SMS: the actual conversation notification with real content.
...
**SMS content:** `SmsReceiver` has been removed. SMS notifications are handled by `NotificationSyncService` like any other notification, but `postSbn()` detects the default SMS app and enriches the entry by fetching `SmsData(body, sender)` from the Telephony content provider via an **immediate** `fetchSms(sbn.postTime)` call at the start of `postSbn` — before the server POST is made. This is the primary defence against Android 15's OTP redaction (which replaces both title and body with generic strings before the NLS sees them) and is fast enough to handle notifications that are auto-dismissed in milliseconds. `enrichSmsBody` is kept as a fallback for devices where the content provider write lags. Requires `READ_SMS` permission — **requested at runtime** in `MainActivity` via a dedicated status row and button. Without it `fetchSms()` returns null; when the notification title is also blank (Android 15 redaction pattern), the notification is skipped entirely rather than forwarding redacted content.
